...
export default class Tabs extends React.Component {
  ...
  async componentDidUpdate(prevProps) {
    if (this.props.initialIndex !== prevProps.initialIndex) {
      setTimeout(() => {
        this.selectMenu(this.props.initialIndex);
      }, 1000)
    }
  }
  ...
  selectMenu = (_id) => {
    ...
    this.menuRef.current.scrollToIndex({
      index: this.props.data.findIndex(item => item._id === _id),
      viewPosition: 0.5
    });

    this.animate();
    this.props.onChange && this.props.onChange(_id);
  }

  renderItem = (item) => {
    const isActive = this.state.active === item._id;
    ...
    return (
      <TouchableOpacity style={containerStyles} onPress={() => this.selectMenu(item._id)}>
          <Animated.Text
